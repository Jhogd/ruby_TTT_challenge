...
class Game_Logic
  ...
  def section_win?(data, player)
    data.all? { |cell| cell == player}
  end
  ...
  def check_win_diags(board, player)
    section_size = board_size
    diag = (0..section_size).collect { |i| board[i][i] }
    inverse_diag = (0..section_size).collect { |i| board[i][-i - 1] }
